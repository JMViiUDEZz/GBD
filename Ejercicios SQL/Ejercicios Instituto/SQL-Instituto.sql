...
-- 16.	Nota media del alumno Emilio Parra Sanz en la asignatura Planificación y Admon de Redes.
SELECT AVG(NOTA) FROM NOTAS NATURAL JOIN ALUMNOS WHERE NOMBRE='Emilio' AND APELLIDOS='Parra Sanz' AND CODAS=(SELECT CODAS FROM ASIGNATURAS WHERE NOMBRE='Planificación y Admon de Redes')
...
-- 18.	Relación de asignaturas de 2º ASIR.
SELECT * FROM ASIGNATURAS WHERE CURSO='2' AND NIVEL='ASIR'
-- 19.	Relación de asignaturas de 2º ASIR que no hayan tenido ningún alumno matriculado nunca.
SELECT * FROM ASIGNATURAS WHERE CURSO='2' AND NIVEL='ASIR' AND CODAS NOT IN (SELECT CODAS FROM MATRICULA)
-- 20.	Relación de asignaturas de 2º ASIR que no tengan ningún alumno matriculado este curso.
SELECT * FROM ASIGNATURAS WHERE CURSO='2' AND NIVEL='ASIR' AND CODAS NOT IN (SELECT CODAS FROM MATRICULA WHERE CURSO='21-22')
-- 21.	Relación de asignaturas en las que no se ha matriculado ningún alumno este curso.
SELECT * FROM ASIGNATURAS WHERE CODAS NOT IN (SELECT CODAS FROM MATRICULA WHERE CURSO='21-22')
-- 22.	¿Cuáles han sido las notas más alta y más baja en Gestión de bases de datos este curso?
SELECT MAX(NOTA),MIN(NOTA) FROM NOTAS WHERE FECHA BETWEEN '2021-09-15' AND '2022-06-22' AND CODAS=(SELECT CODAS FROM ASIGNATURAS WHERE NOMBRE='Gestión de bases de datos')
-- 23.	Alumnos a los que han correspondido las notas más alta y más baja en Gestión de bases de datos este curso.
...
-- 24.	Relación de asignaturas en las que está matriculada este curso la alumna María Sánchez Viúdez.
SELECT * FROM ALUMNOS AL NATURAL JOIN MATRICULA M INNER JOIN ASIGNATURAS A ON A.CODAS=M.CODAS WHERE AL.NOMBRE='María' AND AL.APELLIDOS='Sánchez Viúdez' AND M.CURSO='21-22'
-- 25.	Relación de asignaturas que impartió el profesor Juan Antonio Cuello el curso pasado ordenada.
...
-- 26.	Calificación media de los alumnos que tienen una nota media mayor de 5.
...
-- 28.	Número de alumnos matriculados en las asignaturas de 1º SMyR este curso.
...
-- 29.	Relación de profesores que hayan impartido alguna vez la asignatura de matemáticas en 1º de ESO.
...
-- 30.	Igual que la anterior, pero mostrando el curso académico en que la han impartido.
...
-- 31.	¿Cuántas calificaciones tenemos de cada alumno en la base de datos? Queremos que aparezca el nombre del alumno y el número de calificaciones.
SELECT CODAL,NOMBRE,COUNT(NOTA) "NUMERO CALIFICACIONES" FROM NOTAS NATURAL JOIN ALUMNOS GROUP BY CODAL,NOMBRE
-- 32.	Lo mismo que la anterior, pero mostrando también los alumnos de los que no tengamos ninguna calificación.
...
-- 33.	Relación de profesores que ganan más que todos los profesores de informática.
SELECT * FROM PROFESORES WHERE SUELDO > (SELECT MAX(SUELDO) FROM PROFESORES WHERE DEPARTAMENTO='INFORMATICA')
-- 34.	Relación de profesores que ganan más que alguno de los profesores de informática.
SELECT * FROM PROFESORES WHERE SUELDO > (SELECT MIN(SUELDO) FROM PROFESORES WHERE DEPARTAMENTO='INFORMATICA')
-- 35.	Incrementa en un 10% el sueldo de los profesores de informática.
UPDATE PROFESORES SET SUELDO=SUELDO*1.1 WHERE DEPARTAMENTO='INFORMATICA'
-- 36.	Relación de alumnos y asignatura a los que da clase este curso el profesor Juan Valera Ortega ordenada por asignatura y alumno.
...
-- 37.	Aumenta en 0,1 puntos las calificaciones de Gestión de bases de datos del alumno Emilio Parra Sanz.
UPDATE NOTAS SET NOTA=NOTA+0.1 WHERE CODAS=(SELECT CODAS FROM ASIGNATURAS WHERE NOMBRE='Gestión de bases de datos') AND CODAL=(SELECT CODAL FROM ALUMNOS WHERE NOMBRE='Emilio' AND APELLIDOS='Parra Sanz')
-- 38.	Elimina los alumnos que no tengan calificaciones.
DELETE FROM ALUMNOS WHERE CODAL NOT IN (SELECT CODAL FROM NOTAS)
-- 39.	Alumnos que aprobaron todos los exámenes de Gestión de bases de datos.
SELECT * FROM ALUMNOS A WHERE NOT EXISTS (SELECT * FROM NOTAS WHERE NOTA<5 AND CODAS = (SELECT CODAS FROM ASIGNATURAS WHERE NOMBRE='Gestión de bases de datos') AND CODAL=A.CODAL) AND CODAL IN (SELECT CODAL FROM NOTAS WHERE CODAS = (SELECT CODAS FROM ASIGNATURAS WHERE NOMBRE='Gestión de bases de datos'))
-- 40.	Alumnos matriculados en todas las asignaturas de 1º ASIR.
