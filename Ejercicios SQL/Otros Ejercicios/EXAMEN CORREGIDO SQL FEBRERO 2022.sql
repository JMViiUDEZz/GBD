...
ADD FOREIGN KEY(DIR) REFERENCES EMPLE(EMP_NO) ON DELETE CASCADE ON UPDATE CASCADE; #MAL
...
-- 7.
SELECT D.DNOMBRE,D.LOC,COUNT(EMP_NO) "NUMERO EMPLEADOS" FROM DEPART D RIGHT JOIN EMPLE E ON E.DEPT_NO=D.DEPT_NO GROUP BY DEPT_NO; #MAL

-- 8.
SELECT E.APELLIDO,OFICIO,SALARIO,D.APELLIDO FROM EMPLE E NATURAL JOIN EMPLE D WHERE YEAR(FECHA_ALT)='1990'; #MAL
...
-- 15. 
SELECT DISTINCT NOMBRECLIENTE FROM CLIENTES NATURAL JOIN PEDIDOS WHERE FechaEsperada>FechaEntrega; #MAL
...
-- 20.
SELECT C.CODIGOEMPLEADOREPVENTAS,E.NOMBRE,COUNT(C.CODIGOCLIENTE) "NUMERO CLIENTES" FROM CLIENTES C INNER JOIN EMPLEADOS E ON E.CODIGOEMPLEADO=C.CODIGOEMPLEADOREPVENTAS GROUP BY C.CODIGOEMPLEADOREPVENTAS HAVING COUNT(C.CODIGOCLIENTE)>2;